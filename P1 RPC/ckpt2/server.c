...
int main(int argc, char**argv) {
	...
	while (1) {
		...
		if (sessfd<0) err(1,0);
		rv=recv(sessfd, buf, sizeof(int), 0); // cannot work when put into while loop, wait forever
		...
		rv=recv(sessfd, buf, total_bytes, 0);
		...
		if (cmd == OPEN) {
			...
			int pathname_len;
			memcpy(&pathname_len, buf + 2 * sizeof(int), sizeof(int));
			fprintf(stderr, "pathname_len: %d \n", pathname_len);

			int size = (pathname_len + 1) * sizeof(char);
			char *pathname = malloc(size);
			memcpy(pathname, buf + 3 * sizeof(int), size);
			fprintf(stderr, "pathname: %s \n", pathname);

			int flags;
			memcpy(&flags, buf + 3 * sizeof(int) + size, sizeof(int));

			mode_t m;
			memcpy(&m, buf + 4 * sizeof(int) + size, sizeof(mode_t));
			...
			// put fd with offset:
			memcpy(return_buf + 2 * sizeof(int), (char*)&fd_OFFSET, sizeof(int));
			...
			int fd;
			memcpy(&fd, buf + 2 * sizeof(int), sizeof(int));
			fd -= OFFSET;

			int write_buffer_len;
			memcpy(&write_buffer_len, buf + 3 * sizeof(int), sizeof(int));
			fprintf(stderr, "write_buffer_len: %d \n", write_buffer_len);

			char* write_buffer = malloc(write_buffer_len * sizeof(char));
			memcpy(write_buffer, buf + 4 * sizeof(int), write_buffer_len);

			size_t count;
			memcpy(&count, buf + 4 * sizeof(int) + write_buffer_len, sizeof(count));
