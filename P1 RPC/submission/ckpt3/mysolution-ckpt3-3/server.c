...
int main(int argc, char**argv) {
	...
	while (1) {
		...
		// fprintf(stderr, "start new sessfd: %d\n", sessfd);
		if (sessfd<0) err(1,0);
		rv = fork();
		// fprintf(stderr, "rv value: %d\n", rv);
		if (rv == 0) {
			// fprintf(stderr, "child start!\n");
			close(sockfd);
			while ((rv = recv(sessfd, buf, sizeof(int), 0)) > 0) {
				// fprintf(stderr, "rv in child: %d\n", rv);
			...
			if (rv<0) err(1,0);
			// fprintf(stderr, "close sessfd: %d\n", sessfd);
			close(sessfd);
			exit(0);
		}
		close(sessfd);
	}
	// fprintf(stderr, "outside close sockfd: %d\n", sockfd);
	// close socket
	close(sockfd);
	return 0;
}
